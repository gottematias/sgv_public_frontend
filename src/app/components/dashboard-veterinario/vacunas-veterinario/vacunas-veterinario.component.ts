...
@Component({
  selector: 'app-vacunas-veterinario',
  imports: [CommonModule, TagModule, ButtonModule, TooltipModule],
  providers: [DialogService],
  templateUrl: './vacunas-veterinario.component.html',
  styleUrl: './vacunas-veterinario.component.css',
})
export class VacunasVeterinarioComponent implements OnInit, OnDestroy {
  vacunas: Vacuna[] = [];
  cargandoVacunas = false;
  private refreshSubscription?: Subscription;
  private refDialog?: DynamicDialogRef;
  private readonly REFRESH_INTERVAL_MS = 300000; // 5 minutes
  ...
  private haVacunasChanged(nuevasVacunas: Vacuna[]): boolean {
    if (this.vacunas.length !== nuevasVacunas.length) {
      return true;
    }

    const vacunasActualesMap = new Map(
      this.vacunas.map((vacuna) => [vacuna.id, vacuna]),
    );
