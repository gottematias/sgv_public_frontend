...
@Component({
  selector: 'app-turnos-veterinario',
  imports: [
    CommonModule,
    FormsModule,
    DatePickerModule,
  ...
  templateUrl: './turnos-veterinario.component.html',
  styleUrls: ['./turnos-veterinario.component.css'],
})
export class TurnosVeterinarioComponent implements OnInit, OnDestroy {
  ...
  verFichaMascota(turno: Turno): void {
    if (!turno.mascota?.id) {
    ...
    this.refDialog = this.dialogService.open(MascotaFichaCompletaComponent, {
      header: `Ficha de ${turno.mascota.nombre}`,
      width: '70vw',
      modal: true,
      position: 'top',
      closeOnEscape: true,
      ...
      focusOnShow: false,
      data: { mascotaId: turno.mascota.id },
    });
